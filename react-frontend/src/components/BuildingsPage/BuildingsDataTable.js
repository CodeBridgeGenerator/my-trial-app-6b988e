...
const BuildingsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.buildingname}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.address}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.buidlingtype}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "buildings"});
...
<Column field="buildingname" header="Building name" body={pTemplate0} filter={selectedFilterFields.includes("buildingname")} hidden={selectedHideFields?.includes("buildingname")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="buidlingtype" header="Buidlingtype" body={pTemplate2} filter={selectedFilterFields.includes("buidlingtype")} hidden={selectedHideFields?.includes("buidlingtype")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Buildings Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Buildings" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default BuildingsDataTable;